...
import { checkExistOS, delWMSOS, getEndereco, getItemOSOPAberto, getListaBagSD4, getMaxItemOS, getsd4, insWMS_OPMoega, setInsWMSItemOS, setInsWMSOS } from "../../utils/dbExecute";
...
export async function setDespejoProd (params: any) {
    ...
    const lOSAberto = await getItemOSOPAberto(params.op);
    ...
    try {
        ...
        for(let i = 0; i < listaSD4.length; i++) {
            //Verificando a quantidade do empenho e selecionando os lotes
            nQtdEmpenho = Number(listaSD4[i].qtdori);
            //Selecionando os bags e quantidade para serem inseridas nas ordens de despejo
            //Verificando os bags disponíveis em estoque para enviar às OSs
            ...
            for(let b = 0; b < listBag.length; b++) {
                ...
                if(lContinua) {
                    ...
                    if(Number(nSomaBag) <= Number(nQtdEmpenho)) {
                        //Quantidade do empenho ainda superior a soma dos lotes encontrados, leva todo bag
                    ...
                    if(params.listaEnder.length === 1) {
                        ...
                        for(let m = 0; m < params.listaEnder.length; m++) {
                            ...
                            await insWMS_OPMoega(params.op, enderDestino.enderCod, enderDestino.enderTag);
                        }
                    }
                    ...
                    await setInsWMSItemOS(wmsItem); //Gravando o item
                    nQtdIns++;
                }
            }
        }
        ...
        if(nQtdIns === 0) {
            ...
            ret.data = "Sem empenho";
            ret.type = "No data";
            ret.message = "Não foram encontrados registros na WMS_Bag para os empenhos dessa OP!";
            //Deletando o cabeçalho visto que o item não foi encontrado
            await delWMSOS(wmsOS);
            return ret;
        }

        return ret;
    } catch (error) {
        console.error("Erro em setDespejoProd:", error);
        ret.code = 500;
        ret.type = "Error";
        ret.message = "Erro ao processar despejo de produção";
        ret.data = "";
        return ret;
    }
}