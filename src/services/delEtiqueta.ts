...
export async function delEtiqueta(bagTag:String, bagLote:String) {
    try{
        const sqlCons = `SELECT Z71_TAGRF FROM Z71010 WHERE Z71_TAGRF = @bagTag AND Z71_LOTECT=@bagLote`
        //const sqlDelLocal = `DELETE FROM Z71010 WHERE Z71_TAGRF = @BagTag AND Z71_LOTECTL = @bagLote`
        const sqlDelLocal = `UPDATE Z71010 SET Z71_SEQLOT = 99 WHERE Z71_TAGRF = @BagTag AND Z71_LOTECT = @bagLote`
        const sqlDelNet = `UPDATE Z71010 SET D_E_L_E_T_='*', Z71_EMBAL = 'WMSDEL' WHERE Z71_TAGRF = @bagTag AND Z71_LOTECT = @bagLote`
        const cRet = await executeQueryLocal(sqlCons,{bagTag, bagLote}) 
        let lDelLocal = ""
        let lDelNet = ""

        if(cRet.recordset.length>0){
