...
export async function setSiloSaldoValidation (dados: MovSilos): Promise<Retorno | null> {
    ...
    if (!dados.movSiloSaldoAnt) return new Retorno ({ code: 400, type: "error", message: "Campo movSiloSaldoAnt necessário!"});
    if (!dados.movSiloQuant) return new Retorno ({ code: 400, type: "error", message: "Campo movSiloQuant necessário!"});
